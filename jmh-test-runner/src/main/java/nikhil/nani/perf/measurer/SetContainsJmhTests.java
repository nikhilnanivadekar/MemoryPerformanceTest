...
@Fork(2)
public class SetContainsJmhTests
{
    ...
    @Benchmark
    public int ecIntegerContains()
    {
        this.elements.each(each ->
        {
            if (!this.ecInteger.contains(each))
            {
    ...
    @Benchmark
    public int ecStringContains()
    {
        this.elements.each(each ->
        {
            if (!this.ecString.contains(String.valueOf(each)))
            {
                throw new IllegalStateException();
            }
        });
    ...
    @Benchmark
    public int jdkIntegerContains()
    {
        this.elements.each(each ->
        {
            if (!this.jdkInteger.contains(each))
            {
                throw new IllegalStateException();
            }
        });
    ...
    @Benchmark
    public int jdkStringContains()
    {
        this.elements.each(each ->
        {
            if (!this.jdkString.contains(String.valueOf(each)))
            {
                throw new IllegalStateException();
            }
        });
